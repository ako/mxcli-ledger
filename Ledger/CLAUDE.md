...
```bash
./mxcli -p Ledger.mpr    # Correct - uses local binary
```
...
```bash
./mxcli setup mxbuild -p Ledger.mpr    # Auto-download for project's Mendix version
```

After setup, `mx` is at `~/.mxcli/mxbuild/{version}/modeler/mx`. Usage:

```bash
~/.mxcli/mxbuild/*/modeler/mx check Ledger.mpr   # Validate project
./mxcli docker check -p Ledger.mpr               # Alternative (auto-downloads mxbuild)
```
...
```bash
./mxcli -p Ledger.mpr -c "SHOW MODULES"              # List all modules
./mxcli -p Ledger.mpr -c "SHOW STRUCTURE"             # Project overview
./mxcli -p Ledger.mpr -c "SHOW ENTITIES IN MyModule"  # Entities in a module
./mxcli -p Ledger.mpr -c "DESCRIBE ENTITY MyModule.Customer"  # Entity details
```

### Execute an MDL Script File

```bash
./mxcli exec script.mdl -p Ledger.mpr
```
...
./mxcli check script.mdl -p Ledger.mpr --references  # With reference validation
```
...
```bash
./mxcli check script.mdl -p Ledger.mpr --references
```
...
```bash
# Lint the project
./mxcli lint -p Ledger.mpr

# With colored output
./mxcli lint -p Ledger.mpr --color

# List available rules
./mxcli lint -p Ledger.mpr --list-rules

# Output as SARIF
./mxcli lint -p Ledger.mpr --format sarif > results.sarif
```
...
./mxcli report -p Ledger.mpr

# JSON report
./mxcli report -p Ledger.mpr --format json

# HTML report
./mxcli report -p Ledger.mpr --format html
```
