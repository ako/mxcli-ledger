...
Target version **11.12.1**, pre-fetched so the first build of a session is not a
cold download:

```
mxcli setup mxbuild  --version 11.12.1   # ~1.7 GB → ~/.mxcli/mxbuild/11.12.1
mxcli setup mxruntime --version 11.12.1  # ~397 MB → ~/.mxcli/runtime/11.12.1
```

This gives `mx` (the validator) at
`~/.mxcli/mxbuild/11.12.1/modeler/mx` and `mxbuild` beside it.
