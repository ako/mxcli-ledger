...
### 113. ~~MDL cannot set an OData service's association representation, so every published entity fails CE7375~~ — RESOLVED, and the diagnosis was wrong

> **Superseded 2026-08-17, mxcli 5c70014d.** This is not a missing property.
> `PublishAssociations: No` — which this entry set, believing it meant "this
> service publishes no associations" — *selects* the associated-object-id
> representation, and that is what demands the system ID as key. Omitting the
> property takes the default, "as a link", and the service builds. mxcli now
> warns about exactly this (MDL-ODATA06). Verified end to end: see finding 126.
> The original text is kept below because the wrong diagnosis is the finding.
...
## Phase 20 — the OData endpoint, re-tested (2026-08-17)

`ako/mxcli` main carried OData work. Re-running the probe from Phase 16 against
5c70014d closes finding 113 and corrects it.

### 126. A view entity publishes over OData, keyed on a business column — and the earlier diagnosis was wrong

Finding 113 concluded that publishing an OQL view entity needed a service
property MDL could not set. That was wrong. The property MDL *does* set was
being set to the wrong value, and the value's name is the trap:

```
⚠ odata service Ledger.ChartApi: PublishAssociations: No does not mean
  "publish no associations" — it selects the "as an associated object id"
  representation  [MDL-ODATA06]
  → That representation requires the system ID attribute published as the key,
    which MDL cannot express, so mxbuild fails with CE7375 even when the service
    publishes no associations at all. Use Yes for "as a link (recommended)",
    which is also the default when the property is omitted. Do not follow CE7375
    literally and publish the ID instead: object ids are autogenerated and
    differ between test, acceptance and production, so an id in an external
    contract breaks when a consumer changes environment. Key on a business value.
```

**That warning is the fix, and it is a good one.** It names the property, says
what the value actually selects, gives the correct value, and then talks the
reader out of the obvious-but-wrong workaround that CE7375's own wording invites
— publishing the autogenerated ID as a public key. A build error that says
"publish the ID" and a linter that says "do not" is the linter earning its place.

Omitting the property, the service builds and serves. Verified against 2,588
transactions:

| | |
|---|---|
| `mx check` | **0 errors** — CE7375 gone |
| `$metadata` | `<Key><PropertyRef Name="RowKey"/></Key>` — a **business column**, not the system id |
| Types | `Edm.String`, `Edm.Int64`, `Edm.Decimal` — the CE5016 EdmType fix, visible |
| Data | 728 rows, figures matching OQL to the cent (Groceries 2026: 670.61, 652.98, 655.23, 634.22, …) |
| `$filter` / `$orderby` / `$top` | all applied server-side |
| A chart over it | one `200`, 15 KB, 170 marks, no `chartData` bound |

So the recipe that did not exist in Phase 16 is: aggregate in an OQL view, build
the key as a column in that view, publish it, and **do not mention
PublishAssociations**.

Two caveats found while confirming it, both worth knowing before designing
around this:

- **`$apply` returns 400.** Server-side aggregation through the OData
  aggregation extension is not available, so the *view* has to do the grouping.
  That is what this project does anyway, but it means an endpoint cannot be
  re-grouped by the caller — one view per grain.
- **The anonymous `200` is the project, not the service.** This app runs at
  security level Off, so `authentication session` on the service is inert here.
  It would take effect at Prototype or Production. A published endpoint is API
  surface reachable by anything that can reach the app; checking that at Off
